...
<tr class="product_row" data-row_index="{{$row_count}}" @if(!empty($so_line)) data-so_id="{{$so_line->transaction_id}}" @endif>
	<td>
		...
		@php
			...
			if(!empty($so_line) && $action !== 'edit') {
				...
				$unit_price_inc_tax = $so_line->sell_price_inc_tax;
			}
	...
	<td>
		...
		@if(is_array($sub_units) && count($sub_units) > 0)
			<br>
		...
		<input readonly type="hidden" name="products[{{$row_count}}][tax_id]" class="form-control tax_id input_number mousetrap" value="{{$product->tax_id}}">
		<input readonly type="hidden" name="products[{{$row_count}}][tax_amount]" class="form-control pos_unit_price_neto input_number mousetrap" value="{{@num_format($product->default_sell_price)}}">
