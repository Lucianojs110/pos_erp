...
@section('content')
    ...
    <section class="content">
                            ...
                            <div class="tab-pane active " id="product_list_tab">
                                ...
                                <div class="row mb-3">
                                    @can('product.create')
                                        <div class="col-auto">
                                            <a class="tw-dw-btn tw-bg-gradient-to-r tw-from-indigo-600 tw-to-blue-500 tw-font-bold tw-text-white tw-border-none tw-rounded-full pull-right tw-m-2"
                                                href="#" id="updateSelected">
                                                <i class="fa fa-industry"></i> Actualizar productos seleccionados
                                            </a>
                                        </div>

                                        <div class="col-auto">
                                            <a class="tw-dw-btn tw-bg-gradient-to-r tw-from-indigo-600 tw-to-blue-500 tw-font-bold tw-text-white tw-border-none tw-rounded-full pull-right tw-m-2"
                                                href="{{ action([\App\Http\Controllers\ProductController::class, 'massiveUpdate']) }}">
                                                <i class="fa fa-industry"></i> Actualizar precios masivamente
                                            </a>
                                        </div>
        ...
        @include('product.partials.update_select_products')
...
@section('javascript')
    ...
    <script type="text/javascript">
        ...
        $("#updateSelected").on('click', function(e) {
            productsSelected = [];
            e.preventDefault();
            ...
            table.rows().eq(0).each(function(index) {
                ...
                if ($checkbox.is(':checked')) {
                    productsSelected.push($checkbox.val());
                }
            });
        ...
        $('#submitBtn').click(function() {
            if ($('#increment-percent').val() == 0 && $('#typeOfUpdate').val() == 'percentage') {
                alert("Ingrese un % válido distinto de 0");
            } else if ($('#fixed-value').val() == 0 && $('#typeOfUpdate').val() == 'fixed') {
                alert("Ingrese un valor válido distinto de 0");
            } else {
                $("#products-ids").val(JSON.stringify(productsSelected));
                $('#selection-update-form').submit();
            }
        });

        $("#typeOfUpdate").on('change', function() {
        ...
        $('#fixed-value').on("input", function() {
            var endvalue = $(this).val() * 1 + initialValue * 1;
            $('#example-end-value').val(endvalue);
        });
