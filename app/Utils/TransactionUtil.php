...
class TransactionUtil extends Util
{
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        ...
        //Tax Info
        if ($il->show_tax_1 == 1 && !empty($location_details->cuit)) {
            $output['tax_label1'] = !empty($location_details->tax_label_1) ? $location_details->tax_label_1 . ': ' : '';

            $output['tax_info1'] = $location_details->cuit;
            $output['razon_social'] = $location_details->razon_social;
        }
        if ($il->show_tax_2 == 1 && !empty($location_details->tax_number_2)) {
            if (!empty($output['tax_info1'])) {
                $output['tax_info1'] .= ', ';
            }

            $output['tax_label2'] = !empty($location_details->tax_label_2) ? $location_details->tax_label_2 . ': ' : '';

            $output['tax_info2'] = $location_details->tax_number_2;
        }

        //Shop Contact Info
        $output['contact'] = '';
        if ($il->show_mobile_number == 1 && !empty($location_details->mobile)) {
            $output['contact'] .= 'Tel: ' . $location_details->mobile;
        }
        if ($il->show_alternate_number == 1 && !empty($location_details->alternate_number)) {
            if (empty($output['contact'])) {
                $output['contact'] .= 'Tel: ' . $location_details->alternate_number;
            } else {
        ...
        $output['discount_amount'] = $transaction->discount_amount;
        ...
        if ($transaction->discount_type == 'percentage') {
            if ($customer->iva == 'RESPONSABLE INSCRIPTO' || $customer->iva == 'MONOTRIBUTO') {
                $discount = ($transaction->discount_amount / 100) * $totalUnitPrice;
            } else {
        ...
        $output['cae'] = $transaction->cae;
        $output['exp_cae'] = $transaction->exp_cae;
        $output['afip_invoice_date'] = $transaction->afip_invoice_date;
        $output['num_invoice_afip'] = $transaction->num_invoice_afip;
        $output['qrCode'] = $transaction->qrCode;
        $output['tax_number'] = $customer->tax_number;
        $output['type_invoice'] = $transaction->type_invoice;
        $output['tax_id'] = $transaction->tax_id;
        $output['iibb'] = $transaction->total_before_tax * 0.033;
        $output['iva21'] = number_format((($transaction->iva21 * 21) / 100), 2);
        $output['iva10'] = number_format((($transaction->iva10 * 10.5) / 100), 2);
        $output['iva27'] = number_format((($transaction->iva27 * 27) / 100), 2);
        $output['ref_no'] = $transaction->ref_no;
        $output['customer_iva'] = $customer->iva;
        $output['customer_landmark'] = $customer->landmark;
        $output['customer_state'] = $customer->state;
        $output['customer_city'] = $customer->city;
        $output['customer_mobile'] = $customer->mobile;
        $output['neto'] = number_format(($transaction->iva21 + $transaction->iva10 + $transaction->iva27), 2);
        $output['date'] = date("d/m/Y", strtotime($transaction->transaction_date));
        $output['is_quotation'] = $transaction->is_quotation;
